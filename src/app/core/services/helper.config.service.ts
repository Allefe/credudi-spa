...
export class HelperConfigService {
  ...
  public getConfiguracao(chave: string, valor: string, deveTerminarComBarra?: boolean) {
    return this.configService.getConfiguration(chave, valor, deveTerminarComBarra);
  }
  ...
  public getContextoSistema(sistema?: string, ambiente?: Ambiente) {
    if (sistema && sistema !== undefined && sistema !== '') {
      ...
      if (ambiente && ambiente !== undefined) {
        return this.getUrlBackendAmbiente(ambiente) + this.getContextoApp();
      } else {
        return this.getContextoApp();
      }
    }
  }
  ...
  private getContextoApp() {
    return this.getConfiguracao('APP', 'CONTEXTO');
  }
